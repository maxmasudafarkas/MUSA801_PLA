...
# save locally
saveRDS(properties, file = "../data/local/properties.rds")
...
# saveRDS(vacantLand, file = "../data/local/vacantLand.rds")

# OR read locally
vacantLand <- readRDS('../data/local/vacantLandProps.rds')
...
# SAVE locally
# saveRDS(delinquencies, '../data/local/delinquencies.rds')

# OR read locally
delinquencies <- readRDS('../data/local/delinquencies.rds')
...
# SAVE locally
# saveRDS(transfers, file = "../data/local/transfers.rds")
...
# save locally
# saveRDS(permits, file = "../data/local/permits.rds")
...
mapGuide_ggplot <- function() {
  guides(
    colour = guide_legend(
      override.aes = list(size=5)) # this set the size for the point on legend
    ) 
}
...
plotXY <-
  function(featuresNet, ind_var_list, dep_var) {
    dat_by.stop_ACS <-
      featuresNet %>%
...
```{r features scatterplot, fig.width=8, fig.height=8}

# Plot features by categories
# TODO: Improve colors, tags, style, add R2 on chart

plotXY(featuresNet, var_time, "permitCount")
plotXY(featuresNet, var_activities, "permitCount")
plotXY(featuresNet, var_price, "permitCount")
plotXY(featuresNet, var_vacant, "permitCount")
plotXY(featuresNet, var_pop, "permitCount")
plotXY(featuresNet, var_race, "permitCount")
plotXY(featuresNet, var_inc, "permitCount")
plotXY(featuresNet, var_amenities, "permitCount")
...
var_corr_amen <-
  var_corr %>%
  dplyr::select(all_of(var_amenities))
