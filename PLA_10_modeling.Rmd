...
plotDV_histogram <- function(dataset) {
  ...
  for (i in seq_along(nm)) {
    print(
      ggplot(dataset) +
        geom_histogram(aes_string(x=nm[i]), fill = "#18B6C4", color="white") +
        labs(title = paste("Distribution of ", nm[i])))
  }
}

# Functions to get bivariate regression result
...
get_bivReg <- function(dat_dep, dat_ind, method="original") {
  tab <- c()
  rname <- c()
  dep_i = 0
  ind_i = 0
...
plot_XY <- function (ind_var_list, dep_var) {
  dat_by.stop_ACS <- featuresNet %>%
  ...
  print(
    ggplot(dat_by.stop_ACS) +
      ...
      labs(title = paste("relationship between", substitute(dep_var), "and predictor variables"),
           subtitle = "(continous outcomes for numeric variables)") +
      theme(legend.position = "right")
    )
    
    ggsave(paste("visualizations/2nd_presentation/scatterplots/", substitute(ind_var_list), i, ".png", sep=""), #Gillian's path
...
featuresNet <- readRDS("C:/Users/m1861/Desktop/CPLN790_MUSAPracticum/Data_Box/local/featuresNet_modelReady_0326.rds") %>%
  mutate(permitDummy_numeric = ifelse(permitCount>0, 1, 0)) #random forest does not take factor in well, so calculated the dummy again here
```
...
```{r batch scatterplot}
var_time <- c("year", "lag1years", "lag2years")
var_activities <- c("salesCount", "indOwnerDelta")
var_price <- c("meanSqftPrice", "meanPrice", "medRent", "perc_medRentChange1yr", "perc_medRentChange2yr", "meanDebt")
var_vacant <- c("cumulativeVacantArea", "totalVacantLots", "sheriff")
var_pop <- c("pop", "perc_popChange1yr", "perc_popChange2yr")
var_race <- c("perc_white", "perc_whiteChange1yr", "perc_whiteChange2yr")
var_inc <- c("medInc", "perc_medIncChange1yr", "perc_medIncChange2yr", "jobCount")
var_amenities <- c("licenses", "licensesNN", "parks", "parksNN", "schools", "schoolsNN", "transit", "transitNN", "crime", "crimeNN")
...
```{r train test split}
# hold out on 2019
featuresNet19 <- featuresNet %>%
  filter(year==2019)

# train and test on 2016, 2017, 2018
featuresNetrest <- featuresNet %>%
  filter(year %in% c(2016, 2017, 2018))

set.seed(326)
trainIndex <- createDataPartition(featuresNetrest$permitCount, p = .75,
                                  list = FALSE,
                                  times = 1)
train <- featuresNetrest[trainIndex,] 
test <- featuresNetrest[-trainIndex,] # test data within training data for tuning
```
...
biM0 <-
    glm(as.factor(permitDummy) ~ 
          ...
          perc_white + perc_whiteChange2yr + 
          medInc + + perc_medIncChange2yr +
          licenses + parksNN + schoolsNN + transitNN + crime, family="binomial"(link="logit"), 
      data = train)

summary(biM0)

biM0_out <- data.frame(outcome = as.factor(test$permitDummy),
...
cv_result <- #accuracy
  spatialCV %>%
    dplyr::group_by(cvID) %>%
    dplyr::summarize(accuracy = get_accuracy(caret::confusionMatrix(factor(Prediction, levels = allvalues), factor(permitDummy, levels = allvalues)))) %>%
  ungroup()
