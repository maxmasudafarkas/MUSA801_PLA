...
# SUPPLY SIDE
# JOIN properties to parcels to vacantLand as reference to PROPERTIES

xyz_1 <- x %>%
  inner_join(y, by=c('registry_number'='MAPREG'))

xyz_2 <- x %>%
  inner_join(y, by=c('pin'='PIN'))



xyz_1 <- propertiesData %>%
  st_drop_geometry() %>%
  inner_join(parcelsData, by=c('registry_number'='MAPREG'))

xyz_2 <- propertiesData %>%
  st_drop_geometry() %>%
...
xyz <- propertiesData %>%
  left_join(parcelsData, by=c('registry_number'='BASEREG')) %>%
  left_join(vacantLandData, by=c('parcel_number'='OPA_ID'))



# DEMAND SIDE (multiple values per property)
# join properties to permitsData
a_xyz <-  xyz %>% 
  left_join(a, by = c('parcel_number'='opa_account_num'))

# join properties to delinquenciesData
b_xyz <- xyz %>%
  left_join(b, by = c('parcel_number'='opa_number'))

# add transfers data
c_xyz <- xyz %>%
  left_join(c, by = c('parcel_number'='opa_account_num'))
...
# JOINING delinquencies to properties:
