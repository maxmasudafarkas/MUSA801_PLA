...
ggplot() +
  ...
  geom_sf(data = delinquenciesVacantProps, 
          ...
          size = 0.02) +
  scale_color_manual("", values= c(palette[1], palette[4]), labels= c("no", "yes")) +
...
grid.arrange(
    rbind(
      ...
      theme(axis.text.x = element_blank(),
            ...
            panel.grid.major.x = element_blank(),
            strip.background = element_rect(fill = "#ffffff"),
          strip.text.x = element_text(size = 16, color = colors[1])
            ),
    demographics %>%
...
grid.arrange(
  rbind(
    ...
    scale_fill_manual(values = c(color1, color2)) +
    labs(title="Neighborhood effects",
         subtitle = "Number of properties by Amount of Property Ownership",
         x = "",
         y = "properties") +
...
# save gif locally
# anim_save("zoningPermits.gif", animation)
